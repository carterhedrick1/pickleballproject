...
//
// The browser hides the signup form when a game is cancelled or has ended, but the API is
// reachable without the browser, so the same rule is enforced here - inside the per-game lock,
// by services/player-service.js - before any join transition runs. The cutoff matches the
// browser because it is now literally the same function: both call hasGameEnded in
// public/js/central-time.js, which the page loads and this file requires. Signups stay open
// through the game itself and close when it ends (start + duration), because a late "IN" while
