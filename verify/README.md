...
npm run verify:court-images   # the per-court image library and the per-game selection
npm run verify:promotion      # promotion modes
npm run verify:stats          # host stats (no server needed - pure function)
