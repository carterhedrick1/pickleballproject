...
// can send a text (it never touches sms-handler).
...
(async () => {
  ...
  await db.addLocation(null);
  await db.addLocation('   ');
  check((await db.getLocations()).length === before, 'blank / null / whitespace added nothing');

  await db.addLocation(TEST_LOCATION);
  await db.addLocation('TEST COURT ALPHA');
  const withTest = await db.getLocations();
  ...
  await db.recordRosterSighting(HOST, PLAYER, 'Signup Typed Name', 1);
  check((await roster())?.name === 'Signup Typed Name', 'first sighting sets the name');

  await db.upsertRosterEntry(HOST, PLAYER, 'Host Typed Name', 'DUPR-4417', 3.75);
  ...
  await db.recordRosterSighting(HOST, PLAYER, 'Signup Typed Name Again', 1);
  ...
  await db.recordRosterSighting(HOST, PLAYER, 'Whoever', null);
  check((await roster())?.isAndroid === 1, 'null sighting leaves a known Android flag alone');

  await db.recordRosterSighting(HOST, PLAYER, 'Whoever', 0);
  ...
  await db.recordRosterSighting(HOST, OTHER, 'Never Seen On A Phone', null);
  const unknown = (await db.getRosterForHost(HOST)).find((r) => r.playerPhone === OTHER);
  ...
  check((await db.getRosterForHost(PLAYER)).length === 0, "another host's roster is empty");
  check(await db.deleteRosterEntry(HOST, PLAYER) === 1, 'deleting an entry reports one row removed');
  check(await roster() === undefined, 'the deleted entry is gone');

  await cleanupTestRosterAndLocations();
  const cleaned = await db.getLocations();
