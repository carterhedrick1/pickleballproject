...
const PAGES = [
  ...
  { n: 2, key: 'create', name: 'Create a Game', route: '/create.html', lane: 'In the nav', who: 'Organizers',
    note: 'One form in four numbered sections — Where, When, Who, How — then a share panel that replaces it. The court dropdown is filled in from the server, so its options are not in the file.' },
  { n: 3, key: 'game', name: 'Invite Page', route: '/game.html?id=…', lane: 'Link only', who: 'Players',
    note: 'These panels show and hide to make several different screens. Only some are visible at once. The Photos section only appears once the host has added one.' },
  { n: 4, key: 'manage', name: 'Game Management', route: '/manage.html?id=…&token=…', lane: 'Link only', who: 'Host only',
    note: 'The four tabs, plus the header, the notices and the confirm pop-up. Photos live at the bottom of the Details tab rather than in a tab of their own.' },
  { n: 5, key: 'my-games', name: 'My Games', route: '/my-games.html', lane: 'In the nav', who: 'Organizers',
    note: 'Asks for a phone number once, then loads that host’s games from the server. The cards are built by JavaScript, so only the frame, the gate and the empty state are fixed text.' },
  { n: 6, key: 'roster', name: 'Roster', route: '/roster.html', lane: 'In the nav', who: 'Organizers',
    note: 'Everyone the host has played with. The rows are built by JavaScript from the server, so only the frame and the empty state are fixed text.' },
  { n: 7, key: 'stats', name: 'Stats', route: '/stats.html', lane: 'Linked from My Games', who: 'Organizers',
    note: 'Stat cards and top-five lists, all built by JavaScript. The honest caveats about what the numbers can and cannot show are written by the server, not this file.' },
  { n: 8, key: 'lookup', name: 'Find My Games (retired)', route: '/lookup.html', lane: 'Old links only', who: 'Organizers',
    note: 'Now just redirects to My Games, which does the phone lookup itself. The file stays so older texts and bookmarks still work.' },
  { n: 9, key: 'demo', name: 'SMS Consent Demo', route: '/demo.html', lane: 'Unlinked', who: 'Carrier reviewers',
    note: 'Describes a signup flow the app no longer has. Nothing links here.' },
  { n: 10, key: 'privacy', name: 'Privacy Policy', route: '/privacy.html', lane: 'In the footer', who: 'Anyone',
    note: 'Numbered legal sections.' },
  { n: 11, key: 'terms', name: 'Terms of Service', route: '/terms.html', lane: 'In the footer', who: 'Anyone',
    note: 'Numbered legal sections.' },
];
